...
def AVBCalcMaxImageSize(avbtool, footer_type, partition_size, additional_args):
    ...
    additional_args = "--do_not_generate_fec"
...
def BuildVerifiedImage(data_image_path, verity_image_path,
    ...
    if not Append(data_image_path, verity_image_path,
...
def BuildImage(in_dir, prop_dict, out_file, target_out=None):
    ...
    """
    # system_root_image=true: build a system_dm.img that combines the contents of
    ...
    print ("verity_supported = %s" % (verity_supported))
    print ("verity_fec_supported = %s" % (verity_fec_supported))

    # Adjust the partition size to make room for the hashes if this is to be
    # verified.
    if verity_supported and is_verity_partition:
        partition_size = int(prop_dict.get("partition_size"))
        print ("partition_size = %d" % (partition_size))
    ...
    if avb_footer_type:
        ...
        additional_args = prop_dict["avb_add_" +
                                    avb_footer_type + "_footer_args"]
        ...
        print ("partition_size = %d" % (max_image_size))

# build_command = [prop_dict["ext_mkuserimg"]]

    partition_max_image_size = "out/%s_max_data_size"
    partition_max_image_size %= (prop_dict["partition_name"])

    # print ("%s" %(partition_max_image_size))

    size_tmp = "%s"
    size_tmp %= (prop_dict["partition_size"])

    with open(partition_max_image_size, "wb") as f:
        f.write(size_tmp)
    ...
    if fs_type.startswith("ext"):
        ...
        if "partition_size" in prop_dict:
            build_command.extend(["-l", prop_dict["partition_size"]])
        if "mount_point" in prop_dict:
            build_command.extend(["-L", prop_dict["partition_name"]])
        ...
        build_command = [prop_dict["squash_mkuserimg"]]
        build_command.append(in_dir)
        build_command.append(out_file)
        if "comp_type" in prop_dict:
            build_command.extend(["-comp", prop_dict["comp_type"]])
        if "block_size" in prop_dict:
            build_command.extend(["-b", prop_dict["block_size"]])
        if "core_num" in prop_dict:
            build_command.extend(["-processors", prop_dict["core_num"]])
        if "noI" in prop_dict:
            build_command.append("-noI")
        if "noD" in prop_dict:
            build_command.append("-noD")
        if "noF" in prop_dict:
            build_command.append("-noF")
        if "noX" in prop_dict:
            build_command.append("-noX")
        if "all_root" in prop_dict:
            build_command.append("-all-root")
        build_command.append("-no-sparse")
        build_command.append("-no-progress")
        build_command.append("-noappend")
    ...
    try:
        if fs_type.startswith("ext4"):
            ...
            ext4fs_output = out_file
            exit_code = 0
            # print ("exit_code = %d" % (exit_code))
        else:
            (_, exit_code) = RunCommand(build_command)
            if fs_type.startswith("squashfs"):
                squash_img_sz = os.stat(out_file).st_size
                pad_sz = squash_img_sz % 4096
                if pad_sz != 0:
                    RunCommand("dd if=/dev/zero bs=1 count=%d >> %s" %
                               (pad_sz, out_file))
                squash_img_sz = os.stat(out_file).st_size
                # check if squashfs image + verity is larger than partition defined
                if (squash_img_sz + int(prop_dict["verity_size"])) > int(prop_dict["original_partition_size"]):
                    print("Error: squashfs image larger than dm-verity partition!")
                    return False
                # use actual squashfs image size as dm-verity parameters
                prop_dict["partition_size"] = squash_img_sz
                prop_dict["original_partition_size"] = squash_img_sz + \
                    int(prop_dict["verity_size"])
                with open(partition_max_image_size, "wb") as f:
                    f.write("%d" % (squash_img_sz))
    finally:
        if in_dir != origin_in:
    ...
    if (has_reserved_blocks or "partition_headroom" in prop_dict) and fs_type.startswith("ext4"):
        ...
        if "partition_headroom" in prop_dict:
            headroom_blocks = int(prop_dict.get(
...
def ImagePropFromGlobalDict(glob_dict, mount_point):
    ...
    if mount_point == "system":
        ...
        copy_prop("system_squashfs_disable_4k_align",
        ...
        copy_prop("vendor_squashfs_disable_4k_align",
                  "squashfs_disable_4k_align")
