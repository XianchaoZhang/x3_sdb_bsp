...
void print_help(char *name)
{
	...
	printf("%c: IDX_AE_EX\n", IDX_AE_EX);
	printf("%c: IDX_AE_5BIN_HIS_THRESH\n", IDX_AE_5BIN_HIS_THRESH);
	printf("%c: IDX_THREAD\n", IDX_THREAD);
	printf("Y: Help\n");
	printf("X: Exit\n");
	printf("============================================\n");
...
void print_data(int chn, int type, int bytewidth, void *data, int size)
{
	...
	case IDX_RADIAL_SHADING_LUT:
		...
		break;
	case IDX_ISP_GAMMA_EV2:
		printf("===IDX_ISP_GAMMA_EV2===\n");
		break;
	case IDX_AE_5BIN_HIS_THRESH:
		printf("===IDX_AE_5BIN_HIS_THRESH===\n");
		break;
	default:
		break;
	}
...
void *getset_thread(void *arg)
{
    ...
    sprintf(name, "statsRid%d", ctx_id);
	...
	while (!check_end()) {
		...
		clock_gettime(CLOCK_MONOTONIC, &t1);
		HB_ISP_GetVDTTimeOut(ctx_id, 1, -1);
		clock_gettime(CLOCK_MONOTONIC, &t2);
		diff = diff_timespec(&t2, &t1);
		printf("vdt takes %ld:%ld\n", diff.tv_sec, diff.tv_nsec);

		clock_gettime(CLOCK_MONOTONIC, &t1);
		HB_ISP_GetMeteringData(ctx_id, awb, ISP_AWB, 0);
		clock_gettime(CLOCK_MONOTONIC, &t2);
		diff = diff_timespec(&t2, &t1);
		printf("awb takes %ld:%ld\n", diff.tv_sec, diff.tv_nsec);

		clock_gettime(CLOCK_MONOTONIC, &t1);
		HB_ISP_GetMeteringData(ctx_id, ae, ISP_AE, 0);
		clock_gettime(CLOCK_MONOTONIC, &t2);
		diff = diff_timespec(&t2, &t1);
		printf("ae takes %ld:%ld\n", diff.tv_sec, diff.tv_nsec);

		clock_gettime(CLOCK_MONOTONIC, &t1);
		HB_ISP_GetMeteringData(ctx_id, &af, ISP_AF, 0);
		clock_gettime(CLOCK_MONOTONIC, &t2);
		diff = diff_timespec(&t2, &t1);
		printf("af takes %ld:%ld\n", diff.tv_sec, diff.tv_nsec);

		printf("\n");

		if (diff.tv_nsec > 3000000)
			printf("ERROR, af takes too long\n");
	}
...
get_end:
	...
	case IDX_MODCTRL:
		if (!dir) {
			...
			pr_info("module control get value 0x%x\n", ctrl.u64Value);
		}
		break;
	case IDX_REGISTER:
		if (!dir)
	...
	case IDX_ISP_GAMMA_EV2:
	{
		ISP_GAMMA_EV2 GammaT;
		if (!dir) {
			HB_ISP_SetGammaEv2(0, &GammaT);
		} else {
			memset(&GammaT, 0, sizeof(GammaT));
			HB_ISP_GetGammaEv2(0, &GammaT);
			print_data(0, type, WORD, &GammaT, sizeof(GammaT));
		}
	}
		break;
	case IDX_AE_5BIN_HIS_THRESH:
	{
		ISP_5BIN_HIST BinHT = {0x11, 0x21, 0xD1, 0xE1};
		if (!dir) {
			HB_ISP_Set_AE_5bin_Hist(0, &BinHT);
		} else {
			memset(&BinHT, 0, sizeof(BinHT));
			HB_ISP_Get_AE_5bin_Hist(0, &BinHT);
			print_data(0, type, WORD, &BinHT, sizeof(BinHT));
		}
	}
	break;
	...
	case IDX_THREAD:
	{
		...
		pthread_create(&pid, NULL, (void *)getset_thread, &ctx_idx);
		wait_time_loop();
