 ...
 * @param [in] feature_handle handle of a feature
 * @return ::hbrtSuccess, ::hbrtInvalidModelHandle, ::hbrtInvalidFeatureHandle, ::hbrtErrorAccessToEmptyData
 */
...
/**
 * @brief Get the scale value(s) of a given feature
 * @param [out] scale scale of the feature
 * @param [in] feature_handle handle of a feature
 * @return ::hbrtSuccess, ::hbrtInvalidModelHandle, ::hbrtInvalidFeatureHandle, ::hbrtErrorAccessToEmptyData
 */
HBDK_PUBLIC extern hbrt_error_t hbrtGetFeatureScaleValues(const float **scale, hbrt_feature_handle_t feature_handle);

/**
 * @brief Get the length of scale value(s) of a given feature
 * @param [out] num length of scale value(s)
 * @param [in] feature_handle handle of a feature
 * @return ::hbrtSuccess, ::hbrtInvalidModelHandle, ::hbrtInvalidFeatureHandle
 */
HBDK_PUBLIC extern hbrt_error_t hbrtGetFeatureScaleValueNumber(uint32_t *num, hbrt_feature_handle_t feature_handle);

/**
 * Feature quantization type
 */
typedef enum {
  QUANTIZED_BY_NONE,  // no quantization
  QUANTIZED_BY_SHIFT,
  QUANTIZED_BY_SCALE,
} hbrt_feature_quanti_type_t;

/**
 * @brief Get feature quantization type
 * @param [out] type feature quantization type
 * @param [in] feature_handle handle of a feature
 * @return ::hbrtSuccess, ::hbrtInvalidModelHandle, ::hbrtInvalidFeatureHandle
 */
HBDK_PUBLIC extern hbrt_error_t hbrtGetFeatureQuantiType(hbrt_feature_quanti_type_t *type,
                                                         hbrt_feature_handle_t feature_handle);
 ...
 *       3. the ROI can not exceed left and top boundary of the image.
 *       4. the ROI can only exceed right and bottom boundary of the image by one line/column.
 */
...
/**
 * Use just-in-time generated instructions to crop and resize a general tensor to given size
 * @param [out] funccall_buffer generated function call to do the crop and resize.
 * @param [out] generated_funccall_num  generated function call number.
 * @param [in] input_addr input tensor bpu address
 * @param [in] input_valid_dim  input tensor valid dimension
 * @param [in] input_aligned_dim  input tesnor aligned dimension
 * @param [in] input_layout input tensor layout. For bernoulli and bernoulli2 architecture, this API only support
 * NHCW_8W4C.
 * @param [in] p_rois pointer to the roi array
 * @param [in] roi_num number of rois
 * @param [in] dest_w the ROI will be resized to this width.
 * @param [in] dest_h the ROI will be resized to this height.
 * @param [in] output_layout layout of output.
 ...
 * @param [in] march see note.
 * @param [in] interrupt_num the interrupt number to report when function call execution finished.
 * @param [in] keep_hw_ration if true, it will keep the height to width ration of the original ROI. The valid result
 * will be at the left-top corner.
 * @return ::hbrtSuccess, ::hbrtErrorMemoryAllocationFail, ::hbrtErrorInvalidRoi,
 * ::hbrtErrorIllegalMARCH.
 * @note
 *       1. this api will generate BPU instructions and function call(s) for platform specified by march.
 *       2. the ROI can not exceed boundary of the tensor.
 */
HBDK_PUBLIC extern hbrt_error_t hbrtBilinearRoiResizeFeatureMap(
    void **funccall_buffer, uint32_t *generated_funccall_num, bpu_addr_t input_addr, hbrt_dimension_t input_valid_dim,
    hbrt_dimension_t input_aligned_dim, hbrt_layout_type_t input_layout, const hbrt_roi_t *p_rois, uint32_t roi_num,
    uint32_t dest_w, uint32_t dest_h, hbrt_layout_type_t output_layout, bpu_addr_t output_bpu_addr, uint32_t ri_id,
    MARCH march, uint32_t interrupt_num, bool keep_hw_ration);
 ...
 * @return ::hbrtSuccess, ::hbrtErrorInvalidFeatureHandle, ::hbrtErrorInvalidAddress, ::hbrtErrorIllegalElementType
 */
HBDK_PUBLIC extern hbrt_error_t hbrtUnquantizeByScale(float *to_float_data, hbrt_element_type_t from_int_element_type,
                                                      hbrt_dimension_t dim, const float *scales,
                                                      const void *from_int_data);
 ...
 * @return ::hbrtSuccess, ::hbrtErrorInvalidFeatureHandle, ::hbrtErrorInvalidAddress, ::hbrtErrorIllegalElementType
 */
HBDK_PUBLIC extern hbrt_error_t hbrtQuantizeByScale(void *to_int_data, hbrt_element_type_t to_int_element_type,
                                                    hbrt_dimension_t dim, const float *scales,
                                                    const float *from_float_data);
...
/**
 * Calculate the crc32 of each outputs. Return error if crc32 or number of outputs does not match expected,
 * if expected_crc is provided.
 * @param actual_crc The actual crc32 of each outputs. Caller needs to this when this is no longer used.
 * @param ri_id
 * @param expect_crc Expected number of outputs. If NULL, no crc32 check will be performed.
 * @param expect_num_outputs Expected number of outputs. This argument is unused if expect_crc is NULL
 * @param dump_prefix if not NULL, dump to output with mismatch to file
 * @return ::hbrtSuccess, ::hbrtErrorOutputCrc32VerifyFail
 */
HBDK_PUBLIC hbrt_error_t hbrtGetOutputCrcAndVerify(int32_t **actual_crc, uint32_t ri_id, const int32_t *expected_crc,
                                                   int32_t expect_num_outputs, const char *dump_prefix);
/**
