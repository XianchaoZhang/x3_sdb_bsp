			    ...
			    int width, int height, int stride)
{
	...
	if (width == stride) {
		...
		for (i = 0; i < height / 2; i++) {
			memcpy(buffer + size + i * width, srcBuf1 + i * stride,
			       width);
		}
	}
...
void dump_pym_to_files(pym_buffer_t * pvio_image, int chn, int layer)
{
	...
	dumpToFile2plane(file_name, pvio_image->pym[layer].addr[0],
			 pvio_image->pym[layer].addr[1],
			 pvio_image->pym[layer].width *
			 pvio_image->pym[layer].height,
			 pvio_image->pym[layer].width *
			 pvio_image->pym[layer].height / 2,
			 pvio_image->pym[layer].width,
			 pvio_image->pym[layer].height,
			 pvio_image->pym[layer].stride_size);
	count++;
...
void dump_nv12_to_file(const char *file_name, address_info_t * addr_info)
{
	...
	outfile.write(addr_info->addr[1],
		      addr_info->width * addr_info->height / 2);
	outfile.close();
}
			...
			*sizelength = (*p++ & 0x3) + 1;
			...
			while (numOfArrays--) {
				...
				for (i = 0; i < numNalus; i++) {
					...
					// if(i == 0)
					{
						memcpy(pbHeader + offset,
						       nalu_header, 4);
						offset += 4;
						memcpy(pbHeader + offset, p,
						       nalUnitLength);
		  ...
		  av_param_t * av_param, vp_param_t * vp_param)
{
	...
	if (error < 0) {
		...
		if (av_param->firstPacket) {
			...
			seqHeader =
			    (uint8_t *) malloc(codec->extradata_size + 1024);
			if (seqHeader == NULL) {
				printf("Failed to mallock seqHeader\n");
				return -1;
			}
			memset((void *)seqHeader, 0x00,
			       codec->extradata_size + 1024);
