...
int hb_vp_alloc(vp_param_t * param)
{
	...
	for (auto i = 0; i < vp_param->mmz_cnt; i++) {
		auto ret = HB_SYS_Alloc(&vp_param->mmz_paddr[i],
					(void **)&vp_param->mmz_vaddr[i],
					vp_param->mmz_size);
		if (!ret) {
			printf("mmzAlloc paddr = 0x%x, vaddr = 0x%x i = %d \n",
			       vp_param->mmz_paddr[i], vp_param->mmz_vaddr[i],
			       i);
		} else {
...
int hb_vp_free(vp_param_t * param)
{
	...
	for (auto i = 0; i < vp_param->mmz_cnt; i++) {
		auto ret =
		    HB_SYS_Free(vp_param->mmz_paddr[i], vp_param->mmz_vaddr[i]);
		if (ret == 0) {
