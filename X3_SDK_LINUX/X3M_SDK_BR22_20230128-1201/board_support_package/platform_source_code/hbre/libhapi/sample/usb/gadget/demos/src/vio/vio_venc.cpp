...
int hb_venc_init(vencParam_t * vencparam)
{
	...
	if (vencparam->type == PT_MJPEG) {
		...
		vencChnAttr.stRcAttr.stH264Vbr.u32FrameRate = 25;
		vencChnAttr.stVencAttr.stAttrH264.h264_profile =
		    HB_H264_PROFILE_MP;
	...
	if (vencparam->type == PT_MJPEG) {
		...
		case VENC_RC_MODE_H264CBR:
			{
				...
				pstRcParam->stH264Cbr.u32InitialRcQp =
				    g_intra_qp;
				pstRcParam->stH264Cbr.bMbLevelRcEnable =
				    HB_FALSE;
		...
		case VENC_RC_MODE_H264AVBR:
			{
				pstRcParam->stH264AVbr.u32BitRate =
				    vencparam->bitrate;
				pstRcParam->stH264AVbr.u32FrameRate = 25;
				...
				pstRcParam->stH264AVbr.bMbLevelRcEnable =
				    HB_FALSE;
		...
		case VENC_RC_MODE_H264FIXQP:
			{
				pstRcParam->stH264FixQp.u32FrameRate = 30;
				pstRcParam->stH264FixQp.u32IntraPeriod = 30;
		...
		case VENC_RC_MODE_H264QPMAP:
			{
				...
				pstRcParam->stH264QpMap.u32FrameRate = 30;
				pstRcParam->stH264QpMap.u32IntraPeriod = 30;
				pstRcParam->stH264QpMap.u32QpMapArrayCount =
				    ((width + 0x0f) & ~0x0f) / 16 * ((height +
								      0x0f) &
								     ~0x0f) /
				    16;
		...
		case VENC_RC_MODE_H265CBR:
			{
				...
				pstRcParam->stH265Cbr.bCtuLevelRcEnable =
				    HB_FALSE;
...
void venc_thread(void *param)
{
	...
	printf("venc_thread%d entered\n", vencparam->veChn);
	while (running) {
		memset(pvio_image, 0, sizeof(pym_buffer_t));
		if (!vencparam->bind_flag) {
			ret =
			    HB_VPS_GetChnFrame(vencparam->vpsGrp,
					       vencparam->vpsChn,
					       reinterpret_cast <
			...
			if (vencparam->need_dump) {
				dump_pym_to_files(pvio_image, vencparam->veChn,
						  0);
			}
			HB_VENC_SendFrame(vencparam->veChn, &pstFrame, 0);
			if (vencparam->need_vo) {
				if (pstFrame.stVFrame.width == 3840) {
					hb_vo_send_frame(pvio_image->pym[1]);
				} else {
					hb_vo_send_frame(pvio_image->pym[0]);
				}
			}

			ret =
			    HB_VENC_GetStream(vencparam->veChn, &pstStream,
					      2000);
			...
			if (ret < 0) {
				...
				HB_VENC_ReleaseStream(vencparam->veChn,
						      &pstStream);
			}
			HB_VPS_ReleaseChnFrame(vencparam->vpsGrp,
					       vencparam->vpsChn,
					       reinterpret_cast <
					       void *>(pvio_image));
		} else {
