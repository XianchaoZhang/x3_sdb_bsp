				...
				MIPI_SENSOR_INFO_S * pstSnsAttr)
{
	...
	case OS8A10_30FPS_3840P_RAW10_DOL2:
		memcpy(pstSnsAttr, &SENSOR_OS8A10_30FPS_10BIT_DOL2_INFO,
		       sizeof(MIPI_SENSOR_INFO_S));
		break;
	case F37_30FPS_1080P_RAW10_LINEAR:
		memcpy(pstSnsAttr, &SENSOR_F37_30FPS_10BIT_LINEAR_INFO,
		       sizeof(MIPI_SENSOR_INFO_S));
		break;
	case S5KGM1SP_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstSnsAttr, &SENSOR_S5KGM1_30FPS_10BIT_LINEAR_INFO,
		       sizeof(MIPI_SENSOR_INFO_S));
		break;
	case SC8238_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstSnsAttr, &SENSOR_SC8238_30FPS_10BIT_LINEAR_INFO,
		       sizeof(MIPI_SENSOR_INFO_S));
		break;
...
int SAMPLE_MIPI_GetMipiAttrBySns(MIPI_SNS_TYPE_E enSnsType,
				 MIPI_ATTR_S * pstMipiAttr, vio_cfg_t cfg,
				 int index)
{
	switch (enSnsType) {
	case OS8A10_30FPS_3840P_RAW10_LINEAR:
	...
	case S5KGM1SP_30FPS_2160P_RAW10_LINEAR:
		if (cfg.need_clk[index] == 1) {
			memcpy(pstMipiAttr,
			       &MIPI_SENSOR_S5KGM1_30FPS_10BIT_LINEAR_SENSOR_CLK_ATTR,
			       sizeof(MIPI_ATTR_S));
		} else {
			memcpy(pstMipiAttr,
			       &MIPI_SENSOR_S5KGM1_30FPS_10BIT_LINEAR_ATTR,
			       sizeof(MIPI_ATTR_S));
		}
		break;
	case SC8238_30FPS_2160P_RAW10_LINEAR:
	...
	case IMX415_30FPS_2160P_RAW10_LINEAR:
		if (cfg.need_clk[index] == 1) {
			...
			memcpy(pstMipiAttr,
			       &MIPI_SENSOR_IMX415_30FPS_10BIT_LINEAR_ATTR,
			       sizeof(MIPI_ATTR_S));
		}
		break;
	default:
			       ...
			       VIN_DEV_ATTR_S * pstDevAttr)
{
	...
	case F37_30FPS_1080P_RAW10_LINEAR:
		...
		break;
	case S5KGM1SP_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstDevAttr, &DEV_ATTR_S5KGM1_LINEAR_BASE,
		       sizeof(VIN_DEV_ATTR_S));
		break;
	case SC8238_30FPS_2160P_RAW10_LINEAR:
				...
				VIN_PIPE_ATTR_S * pstPipeAttr)
{
	...
	case F37_30FPS_1080P_RAW10_LINEAR:
		...
		break;
	case S5KGM1SP_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstPipeAttr, &PIPE_ATTR_S5KGM1_LINEAR_BASE,
		       sizeof(VIN_PIPE_ATTR_S));
		break;
	case SC8238_30FPS_2160P_RAW10_LINEAR:
			       ...
			       VIN_DIS_ATTR_S * pstDisAttr)
{
	...
	case OS8A10_30FPS_3840P_RAW10_DOL2:
	case IMX415_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstDisAttr, &DIS_ATTR_OS8A10_BASE,
		       sizeof(VIN_DIS_ATTR_S));
		break;
	default:
		pr_err("not surpport sensor type\n");
		return -1;
			       ...
			       VIN_LDC_ATTR_S * pstLdcAttr)
{
	...
	case OS8A10_30FPS_3840P_RAW10_DOL2:
	case IMX415_30FPS_2160P_RAW10_LINEAR:
		memcpy(pstLdcAttr, &LDC_ATTR_OS8A10_BASE,
		       sizeof(VIN_LDC_ATTR_S));
		break;
	default:
		pr_err("not surpport sensor type\n");
		return -1;
...
int hb_sensor_init(int devId, vio_cfg_t cfg, IOT_VIN_ATTR_S * vin_attr)
{
	...
	vin_attr->snsinfo =
	    static_cast <
	    MIPI_SENSOR_INFO_S * >(malloc(sizeof(MIPI_SENSOR_INFO_S)));
	if (vin_attr->snsinfo == NULL) {
		pr_err("snsinfo malloc error\n");
		return -1;
	}
	memset(vin_attr->snsinfo, 0, sizeof(MIPI_SENSOR_INFO_S));
	SAMPLE_MIPI_GetSnsAttrBySns(static_cast < MIPI_SNS_TYPE_E >
				    (cfg.sensor_id[devId]), vin_attr->snsinfo);
	...
	HB_MIPI_SensorBindSerdes(vin_attr->snsinfo, cfg.serdes_index[devId],
				 cfg.serdes_port[devId]);
		...
		vio_cfg_t cfg, IOT_VIN_ATTR_S * vin_attr)
{
	...
	vin_attr->devinfo =
	    static_cast < VIN_DEV_ATTR_S * >(malloc(sizeof(VIN_DEV_ATTR_S)));
	...
	vin_attr->devexinfo =
	    static_cast <
	    VIN_DEV_ATTR_EX_S * >(malloc(sizeof(VIN_DEV_ATTR_EX_S)));
	if (vin_attr->devinfo == NULL) {
		pr_err("devexinfo malloc error\n");
		return -1;
	}
	vin_attr->pipeinfo =
	    static_cast < VIN_PIPE_ATTR_S * >(malloc(sizeof(VIN_PIPE_ATTR_S)));
	if (vin_attr->pipeinfo == NULL) {
		pr_err("pipeinfo malloc error\n");
		return -1;
	}
	vin_attr->disinfo =
	    static_cast < VIN_DIS_ATTR_S * >(malloc(sizeof(VIN_DIS_ATTR_S)));
	if (vin_attr->disinfo == NULL) {
		pr_err("disinfo malloc error\n");
		return -1;
	}
	vin_attr->ldcinfo =
	    static_cast < VIN_LDC_ATTR_S * >(malloc(sizeof(VIN_LDC_ATTR_S)));
	if (vin_attr->ldcinfo == NULL) {
		pr_err("ldcinfo malloc error\n");
		return -1;
	}
	vin_attr->mipi_attr =
	    static_cast < MIPI_ATTR_S * >(malloc(sizeof(MIPI_ATTR_S)));
	...
	SAMPLE_VIN_GetLdcAttrBySns(static_cast < MIPI_SNS_TYPE_E > (sensor_id),
				   vin_attr->ldcinfo);
	SAMPLE_VIN_GetDevAttrExBySns(static_cast < MIPI_SNS_TYPE_E >
				     (sensor_id), vin_attr->devexinfo);
	SAMPLE_MIPI_GetMipiAttrBySns(static_cast < MIPI_SNS_TYPE_E >
				     (sensor_id), vin_attr->mipi_attr, cfg,
				     pipeId);

	ret = HB_SYS_SetVINVPSMode(pipeId,
				   static_cast < SYS_VIN_VPS_MODE_E >
