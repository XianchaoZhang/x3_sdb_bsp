...
void alsa_device_debug_enable(int enable)
{
	...
	if ((dump_hw_params && enable) || (!dump_hw_params && !enable))
		return;
...
{
	alsa_device_t *adev = (alsa_device_t *)calloc(1, sizeof(alsa_device_t));
		...
		snd_pcm_access_t access)
{
	...
	if (adev->period_size > 0) {
		printf(_("Requested period size %lu frames\n"), adev->period_size);
		err = snd_pcm_hw_params_set_period_size_near(handle, params, &adev->period_size, NULL);
		if (err < 0) {
			fprintf(stderr, _("Unable to set period size %lu frames for playback: %s\n"),
	...
	if (adev->period_time > 0) {
		...
		err = snd_pcm_hw_params_set_period_time_near(handle, params, &adev->period_time, NULL);
		if (err < 0) {
	...
	if (adev->buffer_time > 0) {
		...
		err = snd_pcm_hw_params_set_buffer_time_near(handle, params, &adev->buffer_time, NULL);
		if (err < 0) {
	...
	if (!adev->buffer_time && !adev->period_time && !adev->period_size) {
		...
		err = snd_pcm_hw_params_set_buffer_size_near(handle, params, &adev->buffer_size);
		if (err < 0) {
	...
	if (!adev->buffer_time || !adev->period_time) {
		printf(_("Periods = %u\n"), adev->nperiods);
		err = snd_pcm_hw_params_set_periods_near(handle, params, &adev->nperiods, NULL);
		if (err < 0) {
	...
	if (dump_hw_params) {
		fprintf(stderr, _("Applied HW Params of device \"%s\":\n"),
			snd_pcm_name(handle));
		fprintf(stderr, "--------------------\n");
		snd_pcm_hw_params_dump(params, log);
		fprintf(stderr, "--------------------\n");
...
int alsa_device_read(alsa_device_t *adev, void *buffer, snd_pcm_uframes_t frames)
{
	int rc;
	...
	printf("%s snd_pcm_readi begin. handle(%p), name(%s), frames(%d)\n",
			__func__, adev->handle, adev->name, (int)frames);
#endif
...
int alsa_device_write(alsa_device_t *adev, void *buffer, snd_pcm_uframes_t frames)
{
	int rc;
