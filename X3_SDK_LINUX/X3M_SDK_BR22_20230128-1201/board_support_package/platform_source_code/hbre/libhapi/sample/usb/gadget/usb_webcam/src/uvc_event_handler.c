...
#include "extension_unit_event.h"
...
/* ---------------------------------------------------------------------------
 * A.9.7. Extension Unit Control Selectors - Defined By Customer
 */
struct uvc_cs_ops extension_unit_cs_ops[] = {
	{UVC_XU_CONTROL_UNDEFINED, 0, 0, 0},
	{UVC_XU_FIRMWARE_VERSION, sizeof(int32_t) , 0, 0},
};
...
#define UVC_XU_CS_OPS_NUM (sizeof(extension_unit_cs_ops)/sizeof(extension_unit_cs_ops[0]))
	...
	uvc_setup_callback setup_f, uvc_data_callback data_f, int data_len)
{
	...
	case UVC_CTRL_EXTENSION_UNIT_ID:
		if (cs > UVC_XU_CS_OPS_NUM - 1) {
			goto err;
		} else {
			extension_unit_cs_ops[cs].setup_phase_f = setup_f;
			extension_unit_cs_ops[cs].data_phase_f = data_f;
		}
		break;
...
static void* uvc_get_cs_ops(uint8_t cs, uint8_t id)
{
	...
	case UVC_CTRL_PROCESSING_UNIT_ID:
		...
		break;
	case UVC_CTRL_EXTENSION_UNIT_ID:
		if (cs < UVC_XU_CS_OPS_NUM)
			ops = &extension_unit_cs_ops[cs];
		break;
	default:
		break;
	}
...
/* ---------------------------------------------------------------------------
 * Camera Terminal Setup&Data Handler - A.9.4. Camera Terminal Control Selectors
 */
			   ...
			   void *userdata)
{
	...
	case UVC_GET_CUR:
		 ...
		 * 3: aperture_priority
		 * */
		/* get current value form isp or camera*/
		camera_get_aemode(cam, &ae_mode);
		ctrl->AEMode.d8 = ae_mode;
			...
			void *userdata)
{
	...
	switch (req) {
	case UVC_GET_CUR:
		ret = camera_get_exposure(cam, &exposure);
		if(ret) {
	...
	case UVC_GET_INFO:
		resp->data[0] = 0x3;	/* Support Get/Set value requests */
		resp->length = 1;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_INFO val = %d\n",
			__func__, 0x3);
		break;
			...
			void *userdata)
{
	...
	exposure = ctrl->dwExposureTimeAbsolute;
	if(exposure >= 0 &&
		exposure <= g_uvc_param.ct_ctrl_max.dwExposureTimeAbsolute) {
...
/**
 * uvc_ct_zoompos_abs_setup - get the zoompos status value,include current value
 * maximum value,minimum value. etc
 * @ctx: pointer to the struct of the uvc context
 * @resp: pointer to the request data of uvx
**/
void uvc_ct_zoompos_abs_setup(struct uvc_context *ctx,
                       struct uvc_request_data *resp,
                       uint8_t req, uint8_t cs,
                       void *userdata)
{
	...
	ctrl = (union uvc_camera_terminal_control_u*)&resp->data;
	resp->length = sizeof(ctrl->wObjectiveFocalLength);

	memset(ctrl, 0, sizeof(*ctrl));

	switch (req) {
	case UVC_GET_CUR:
		ret = camera_get_zoompos(cam, &zoompos);
		UVC_PRINTF(UVC_LOG_INFO, "#ret = %d, zoompos = %d\n", ret, zoompos);
		if (ret < 0) {
			UVC_PRINTF(UVC_LOG_TRACE, "camera_get_zoompos failed\n");
			ctrl->wObjectiveFocalLength =
			g_uvc_param.ct_ctrl_def.wObjectiveFocalLength;
			break;
		} else {
			g_uvc_param.ct_ctrl_cur.wObjectiveFocalLength = zoompos;
			ctrl->wObjectiveFocalLength =
			g_uvc_param.ct_ctrl_cur.wObjectiveFocalLength;
		}

		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_CUR val = %d\n",
					__func__, ctrl->wObjectiveFocalLength);
		break;

	case UVC_GET_MIN:
		ctrl->wObjectiveFocalLength =
		g_uvc_param.ct_ctrl_min.wObjectiveFocalLength;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_MIN val = %d\n",
					__func__, ctrl->wObjectiveFocalLength);
		break;
	...
	case UVC_GET_MAX:
		ctrl->wObjectiveFocalLength =
		g_uvc_param.ct_ctrl_max.wObjectiveFocalLength;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_MAX val = %d\n",
					__func__, ctrl->wObjectiveFocalLength);
		break;
	...
	case UVC_GET_RES:
		ctrl->wObjectiveFocalLength =
		g_uvc_param.ct_ctrl_res.wObjectiveFocalLength;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_RES val = %d\n",
					__func__, ctrl->wObjectiveFocalLength);
		break;
...
/**
 * uvc_ct_zoompos_abs_data - set the zoompos status value
 * @ctx: pointer to the struct of the uvc context
 * @d: pointer to the request data of uvx
**/
void uvc_ct_zoompos_abs_data(struct uvc_context *ctx,
                       struct uvc_request_data *d, uint8_t cs,
                       void *userdata)
{
	int ret = 0;
	...
	ctrl = (union uvc_camera_terminal_control_u*)&d->data;
	if (d->length != sizeof(ctrl->wObjectiveFocalLength)) {
		UVC_PRINTF(UVC_LOG_ERR,
				"!!! %s return a no vaild data, req len = %d\n",
				__func__, d->length);
		return;
	}

	zoompos = (ctrl->wObjectiveFocalLength);
	if (zoompos <= g_uvc_param.ct_ctrl_max.wObjectiveFocalLength) {
		ret = camera_set_zoompos(cam, zoompos);
		if (ret < 0) {
			UVC_PRINTF(UVC_LOG_ERR, "#camera_set_zoompos failed! "
					"zoompos: %d\n", zoompos);
		} else {
			UVC_PRINTF(UVC_LOG_ERR,
					"#camera_set_zoompos successful! "
					"zoompos: %d\n", zoompos);
			g_uvc_param.ct_ctrl_res.wObjectiveFocalLength = zoompos;
		}
	} else {
		UVC_PRINTF(UVC_LOG_ERR, "### %s [CT][zoompos]: %d\n", __func__,
					zoompos);
	}
}

/* ---------------------------------------------------------------------------
 * Processing Unit Setup&Data Handler - A.9.5. Processing Unit Control Selectors
 */
			   ...
			   void *userdata)
{
	...
	case UVC_GET_INFO: resp->data[0] = 0x3;	/* Support Get/Set value requests */
		resp->length = 1;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_INFO val = %d\n",
			__func__, 0x3);
		break;
...
/* ---------------------------------------------------------------------------
 * Extension Unit Setup&Data Handler - Defined by customer
 */
void uvc_xu_firmware_version_setup(struct uvc_context *ctx,
			   struct uvc_request_data *resp,
			   uint8_t req, uint8_t cs,
			   void *userdata)
{
	union uvc_extension_unit_control_u *ctrl;

	UVC_PRINTF(UVC_LOG_TRACE, "# %s (req %02x)\n", __func__, req);

	ctrl = (union uvc_extension_unit_control_u*)&resp->data;
	resp->length = sizeof(ctrl->dwFirmwareVersion);

	memset(ctrl, 0, sizeof(*ctrl));;

	/*
	 * extension unit must support GET_CUR, GET_MIN, GET_MAX,
	 * GET_RES, GET_INFO, GET_DEF, GET_LEN mandatory.
	 */
	switch (req) {
	case UVC_GET_CUR:
		ctrl->dwFirmwareVersion = 123456789;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_CUR val = %d\n",
			__func__, ctrl->dwFirmwareVersion);
		break;
	...
	case UVC_GET_RES:
		ctrl->dwFirmwareVersion = 0;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_RES val = %d\n",
			__func__, ctrl->dwFirmwareVersion);
		break;

	case UVC_GET_INFO:
		resp->data[0] = 0x1;	/* Support Get value requests */
		resp->length = 1;
		UVC_PRINTF(UVC_LOG_INFO, "# %s UVC_GET_INFO val = %d\n",
			__func__, 0x1);
		break;
...
void uvc_xu_firmware_version_data(struct uvc_context *ctx,
			   struct uvc_request_data *d, uint8_t cs,
			   void *userdata)
{
	UVC_PRINTF(UVC_LOG_TRACE, "# %s not support!\n", __func__);
...
void uvc_control_request_callback_init()
{
	...
	/*
	 * With android vim3 feishu client, uvc bulk mode will meet dwc3 controller -110
	 * issue easily. The reason seems to have relationship with below zoom operation.
	 * The major error log "!!! uvc_ct_zoompos_abs_data return a no vaild data, req len = 0",
	 * which will be analysed later.
	 */
#if 0
	uvc_cs_ops_register(UVC_CTRL_CAMERA_TERMINAL_ID,
		UVC_CT_ZOOM_ABSOLUTE_CONTROL,
		uvc_ct_zoompos_abs_setup,
		uvc_ct_zoompos_abs_data, 0);
#endif
	...
	/* extension unit - customer defined */
	uvc_cs_ops_register(UVC_CTRL_EXTENSION_UNIT_ID,
		UVC_XU_FIRMWARE_VERSION,
		uvc_xu_firmware_version_setup,
		uvc_xu_firmware_version_data, 0);
}
...
void uvc_control_request_param_init()
{
	 ...
	 * */
	g_uvc_param.ct_ctrl_def.dwExposureTimeAbsolute  = 400;
	g_uvc_param.ct_ctrl_cur.dwExposureTimeAbsolute  = 400;
	g_uvc_param.ct_ctrl_res.dwExposureTimeAbsolute  = 10;
	g_uvc_param.ct_ctrl_min.dwExposureTimeAbsolute  = 10;
	/*maximum value of ExposureTimeAbsolute is 2310*/
	g_uvc_param.ct_ctrl_max.dwExposureTimeAbsolute  = 2310;
	...
	/*minimum value of Zoom is 10*/
	g_uvc_param.ct_ctrl_min.wObjectiveFocalLength = 10;
	/*maximum value of Zoom is 200*/
	g_uvc_param.ct_ctrl_max.wObjectiveFocalLength = 200;
	...
	g_uvc_param.pu_ctrl_def.wBrightness = 128;
	g_uvc_param.pu_ctrl_cur.wBrightness = 128;
	g_uvc_param.pu_ctrl_res.wBrightness = 1;
	g_uvc_param.pu_ctrl_min.wBrightness = 0;
	/*maximum value of Brightness is 255*/
	g_uvc_param.pu_ctrl_max.wBrightness = 255;

	g_uvc_param.pu_ctrl_def.wContrast = 128;
	g_uvc_param.pu_ctrl_cur.wContrast = 128;
	g_uvc_param.pu_ctrl_res.wContrast = 1;
	g_uvc_param.pu_ctrl_min.wContrast = 0;
	/*maximum value of contrast is 255*/
	...
	g_uvc_param.pu_ctrl_def.wHue = 128;
	g_uvc_param.pu_ctrl_cur.wHue = 128;
	...
	/*maximum value of saturation is 255*/
	...
	/*maximum value of sharpness is 255*/
				...
				void *userdata)
{
	...
	if (req == UVC_GET_LEN) {
		UVC_PRINTF(UVC_LOG_DEBUG, "# %s cs (%u) UVC_GET_LEN len = %d\n",
				__func__, cs, ops->len);
		len = ops->len;
		memcpy(resp->data, &len, sizeof(len));
		resp->length = sizeof(len);
