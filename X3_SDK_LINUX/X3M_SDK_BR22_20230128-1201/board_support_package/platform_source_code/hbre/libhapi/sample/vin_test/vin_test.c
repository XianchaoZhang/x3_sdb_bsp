...
extern struct hb_vin_group_s *g_vin[6];
...
void parse_opts(int argc, char *argv[])
{
	while (1) {
		static const char short_options[] =
		    "s:i:p:c:r:d:y:t:m:k:b:o:v:f:S:O:M:X:Y:I:P:D:N:F:W:";
...
int md_func(work_info_t * info)
{
	...
	if (ret < 0) {
		...
		printf("HB_VIN_MotionDetect success!!! ret %d \n", ret);
	}
	return ret;
}
...
int vps_get_dump_func(work_info_t * info)
{
	...
	while (check_end()) {
		if (info->data_type != chns2chn(need_pym)) {
			if((info->group_id == 0) && (info->data_type == 2)) {
				  continue;
			}
			frame_cnt++;
...
int main(int argc, char *argv[])
{
	...
	system("devmem 0xa4001460 w 32 32");
	if(need_cam == 1) {
		for(i = 0; i < MAX_ID_NUM; i++) {
			if (BIT(i) & groupMask) {
			    printf("%d===groupMask %d ====sensorId[i] %d ==mipiIdx[i] %d===\n", (BIT(i) & groupMask), groupMask, sensorId[i], mipiIdx[i]);
