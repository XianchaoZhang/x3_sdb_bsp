...
/***** imx415 bv *****/
MIPI_SENSOR_INFO_S SENSOR_4LANE_IMX415_30FPS_10BIT_BV_INFO =
{
    .deseEnable = 0, // 该 sensor 是否有 serdes（串解器）
    .inputMode = INPUT_MODE_MIPI, // sensor 接入方式,mipi还是dvp
    .sensorInfo = {
...
MIPI_ATTR_S MIPI_4LANE_SENSOR_IMX415_30FPS_10BIT_BV_ATTR =
{
    .mipi_host_cfg = {
        .lane = 4, // 硬件上sensor用了几个mipi数据lane，f37用的1 lane, os8a10 4K的用了4 lane
        .datatype = 0x2b, // sensor的输出数据类型，请参考 《X3J3平台AIOT媒体系统接口手册.pdf》第3.5.36节 DATA TYPE
...
VIN_DEV_ATTR_S DEV_ATTR_IMX415_BV_BASE = {
    .stSize = { // 输入的数据
        .format = 0, // 像素格式， format 为 0 代表是raw, 根据 pixel_lenght 来表示究竟是 raw8, raw12 还是 raw16 。
        .width = 3840, // 数据宽
        .height = 2160, // 数据高
...
VIN_PIPE_ATTR_S PIPE_ATTR_IMX415_BV_BASE = {
    .ddrOutBufNum = 8, // ISP->GDC ddr的buf数量，最少可以设置成 2
    .snsMode = NORMAL_M, // sensor 工作模式，要和MIPI_SENSOR_INFO_S配置的sensor_mode一致
    .stSize = { // 数据格式同 VIN_DEV_SIZE_S
        .format = 0,
        .width = 3840,
        .height = 2160,
