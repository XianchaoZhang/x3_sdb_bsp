...
#define F_WIDTH 3264
#define F_HEIGHT 2448

MIPI_SENSOR_INFO_S SENSOR_2LANE_OV8856_10BIT_LINEAR_INFO =
{
	...
	.sensorInfo = {
		...
		.fps = 10, // 帧率，用来选择使用哪一组帧率的sensor参数
		...
		.sensor_name = "ov8856", // sensor的名字，在libcam.so中会根据这个名字组合出 libf37.so
...
MIPI_ATTR_S MIPI_2LANE_OV8856_10BIT_LINEAR_ATTR =
{
	.mipi_host_cfg = {
		...
		.mipiclk = 1272*2, // sensor 输出总的 mipibit rate, 单位Mbits/秒
		.fps = 10, // sensor 输出实际帧率, 和 MIPI_SENSOR_INFO_S 是什么关系？
		.width = F_WIDTH, // sensor实际输出的分辨率，一般根据sensor供应商的配置参数配置就行
		.height = F_HEIGHT,
		.linelenth = 1936, //3872, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
		.framelenth = 3720, // sensor 输出带 blanking的总行数
		.settle = 10, // sensor 输出实际Ttx-zero + Ttx-prepare时间（ clk 为单位）, 范围从0-127，常用的值是20，30，40，50，60，65
...
VIN_DEV_ATTR_S DEV_ATTR_2LANE_OV8856_10BIT_LINEAR_BASE = {
	.stSize = { // 输入的数据
	...
	.outDdrAttr = { // sif(dev) 的输出到 ddr 配置
		.stride = F_WIDTH*1.25, // 硬件 stride 跟格式匹配，通过行像素根据raw数据bit位数计算得来（而且结果要32对齐），8bit：x1, 10bit: x1.25 12bit: x1.5,例F37 raw10，1920 x 1.25 = 2400
...
VIN_PIPE_ATTR_S PIPE_ATTR_2LANE_OV8856_10BIT_LINEAR_BASE = {
	...
	.calib = { // 是否开启 sensor 矫正数据加载，1 是开启，0 是关闭。ISP使用这些数据
		...
		.lname = "libov8856_linear.so", // 对应使用的校准库
	}
};

VIN_DIS_ATTR_S DIS_ATTR_OV8856_BASE = {
  .picSize = {
    .pic_w = F_WIDTH-1,
	.pic_h = F_HEIGHT-1,
  },
...
VIN_LDC_ATTR_S LDC_ATTR_OV8856_BASE = {
    .ldcEnable = 0,
    .ldcPath = {
        .rg_y_only = 0,
        .rg_uv_mode = 0,
