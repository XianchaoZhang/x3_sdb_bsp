...
int imx415_bv_vin_param_init(x3_vin_info_t* vin_info)
{
	vin_info->snsinfo = SENSOR_4LANE_IMX415_30FPS_10BIT_BV_INFO;
	vin_info->mipi_attr = MIPI_4LANE_SENSOR_IMX415_30FPS_10BIT_BV_ATTR;
	vin_info->devinfo = DEV_ATTR_IMX415_BV_BASE;
	vin_info->pipeinfo = PIPE_ATTR_IMX415_BV_BASE;
	vin_info->disinfo = DIS_ATTR_IMX415_BASE;
	vin_info->ldcinfo = LDC_ATTR_IMX415_BASE;
	vin_info->vin_vps_mode = VIN_SIF_ONLINE_DDR_ISP_DDR_VPS_ONLINE;
...
int vps_chn_param_init(x3_vps_chn_attr_t *vps_chn_attr, int chn_id, int width, int height, int fps)
{
	...
	vps_chn_attr->m_chn_attr.frameDepth = 1;/* 图像队列长度, 最大32 */
...
static int vot_param_init_hdmi(x3_vot_info_t *vot_info)
{
	// devAttr
...
static int vot_param_init_mipi_dsi(x3_vot_info_t *vot_info)
{
	// devAttr
	vot_info->m_devAttr.enIntfSync = VOT_OUTPUT_720x1280;
	vot_info->m_devAttr.u32BgColor = 0x108080;
	vot_info->m_devAttr.enOutputMode = HB_VOT_OUTPUT_MIPI;

	// layerAttr
	vot_info->m_stLayerAttr.stImageSize.u32Width = 736;
    vot_info->m_stLayerAttr.stImageSize.u32Height = 1280;
	vot_info->m_stLayerAttr.big_endian = 0;
    vot_info->m_stLayerAttr.display_addr_type = 2;
    vot_info->m_stLayerAttr.display_addr_type_layer1 = 2;
    ...
    vot_info->m_stLayerAttr.panel_type = 0;
    vot_info->m_stLayerAttr.rotate = 0;
    vot_info->m_stLayerAttr.user_control_disp = 1;

	// ChnAttr
	vot_info->m_stChnAttr.u32Priority = 2;
    vot_info->m_stChnAttr.s32X = 0;
    vot_info->m_stChnAttr.s32Y = 0;
    vot_info->m_stChnAttr.u32SrcWidth = 736;
    vot_info->m_stChnAttr.u32SrcHeight = 1280;
    vot_info->m_stChnAttr.u32DstWidth = 736;
    vot_info->m_stChnAttr.u32DstHeight = 1280;

	// cropAttr
	vot_info->m_cropAttrs.u32Width = 720;
    vot_info->m_cropAttrs.u32Height = 1280;
	return 0;
}


int vot_param_init(x3_vot_info_t *vot_info, int is_mipi_dsi)
{
	if (is_mipi_dsi) {
		return vot_param_init_mipi_dsi(vot_info);
	} else {
		return vot_param_init_hdmi(vot_info);
	}
}
