...
{
	printf("venc chn xxx thread exit\n");
}
...
int x3_venc_init(int VeChn, VENC_CHN_ATTR_S *vencChnAttr)
{
    ...
    if (s32Ret != 0) {
        printf("x3_venc_initattr failed\n");
        return -1;
    }

    s32Ret = HB_VENC_SetChnAttr(VeChn, vencChnAttr);  // config
    if (s32Ret != 0) {
        printf("HB_VENC_SetChnAttr failed, s32Ret: %d\n", s32Ret);
        return -1;
    }
...
int x3_venc_reinit(int Vechn, VENC_CHN_ATTR_S *vencChnAttr)
{
    ...
    if (s32Ret != 0) {
        printf("x3_venc_init failed\n");
        return -1;
    }

    s32Ret = x3_venc_start(Vechn);
    if (s32Ret != 0) {
        printf("x3_venc_start failed\n");
        return -1;
    }
...
int x3_venc_set_bitrate(int VeChn, int bitrate)
{
	...
	if (ret) {
		LOGE_print("HB_VENC_GetRcParam failed: %d", ret);
		return ret;
	}
	...
	if (ret) {
		LOGE_print("HB_VENC_SetRcParam failed: %d", ret);
	}
    return ret;
}
