...
{
	printf("normal pipe_id (%d)type(%d)frame_id(%d)buf_index(%d)w x h x s(%d x %d x %d) data_type %d img_format %d\n",
...
static int x3_mipi_deinit(int mipiIdx)
{
	...
	printf("x3_sensor_deinit end==mipiIdx %d====\n", mipiIdx);
...
int x3_vin_init(x3_vin_info_t *vin_info)
{
	...
	if (ret < 0) {
		pr_err("x3_sensor_init error!\n");
		return -1;
	}
	LOGI_print("x3_sensor_init ok!\n");
	ret = x3_mipi_init(vin_info);
	if (ret < 0) {
		pr_err("x3_mipi_init error!\n");
	...
	printf("vin_info->mipi_attr.mipi_host_cfg.channel_num: %d\n", vin_info->mipi_attr.mipi_host_cfg.channel_num);
	...
	LOGI_print("start HB_VIN_SetDevAttr \n");
	...
	LOGI_print("HB_VIN_SetDevAttr ok!\n");
	// 这个接口耗时2.5秒，神奇的一腿
	...
	LOGI_print("HB_VIN_SetPipeAttr ok!\n");
...
int x3_vin_isp_yuv_dump_to_jpeg(VENC_CHN VeChn, int pipeId, char *file_name)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			printf("x3_venc_get_chn_attr error!!!\n");
		}
		// 根据yuv的size设置编码分辨率属性
		...
		if (ret < 0) {
			printf("HB_VENC_SendFrame error, %x\n", ret);
			usleep(100);
			return ret;
		}
		...
		if (ret < 0) {
			printf("HB_VENC_GetStream error, %x\n", ret);
		} else {
			sprintf(file_name,
			...
			if (ret < 0) {
				printf("HB_VENC_ReleaseStream error!!!\n");
			}
		}
		ret = HB_VIN_ReleaseChnFrame(pipeId, 0, isp_yuv);
		if (ret < 0) {
