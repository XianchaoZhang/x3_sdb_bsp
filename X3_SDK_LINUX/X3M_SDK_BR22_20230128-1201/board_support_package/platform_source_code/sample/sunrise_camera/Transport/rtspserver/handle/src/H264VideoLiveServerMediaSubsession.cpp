...
char *shmId, char *shmName, int streamBufSize, int frameRate)
	...
	// 外部的给shm的参数终于传进来了，后面有时间看看怎么传递会更合适吧
	strncpy(fShmId, shmId, strlen(shmId));
	strncpy(fShmName, shmName, strlen(shmName));
...
void H264VideoLiveServerMediaSubsession::seekStream(unsigned /*clientSessionId*/,
	...
	LOGI_print("H264VideoLiveServerMediaSubsession::seekStream1");
...
void H264VideoLiveServerMediaSubsession::seekStream(unsigned /*clientSessionId*/,
	...
	LOGI_print("H264VideoLiveServerMediaSubsession::seekStream2");
	// Seeking isn't allowed if multiple clients are receiving data from the same source:
	if (fReuseFirstSource) return;
...
//	estBitrate = 1500; // kbps, estimate
	...
	LOGI_print("fFrameRate: %d", fFrameRate);
