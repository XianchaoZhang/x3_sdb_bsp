...
PCMAAudioSource* PCMAAudioSource::createNew(UsageEnvironment& env, 
	u_int8_t bitsPerSample, 
	u_int8_t samplingFrequencyIndex, 
	u_int8_t channelConfiguration)
{
	PCMAAudioSource* source = new PCMAAudioSource(env, bitsPerSample, samplingFrequencyIndex, channelConfiguration);
	return source;
}

PCMAAudioSource::PCMAAudioSource(UsageEnvironment& env,
	u_int8_t bitsPerSample, 
	u_int8_t samplingFrequencyIndex, 
	u_int8_t channelConfiguration)
	: FramedSource(env)
{
	...
	/*fPCMAAdoShmSource = fShmSource;*/
	fPts = 0;
}

void PCMAAudioSource::sync()
{
	shm_stream_sync(fShmSource);
}

PCMAAudioSource::~PCMAAudioSource()
{
	if(fShmSource != NULL)
	{
		shm_stream_destory(fShmSource);
		fShmSource = NULL;
		/*fPCMAAdoShmSource = NULL;*/
	}
}


void PCMAAudioSource::doGetNextFrame() {
	//do read from memory
	incomingDataHandler(this);
}

void PCMAAudioSource::incomingDataHandler(PCMAAudioSource* source) {
...
void PCMAAudioSource::incomingDataHandler1()
{
	fFrameSize = 0;
	frame_info info;
	unsigned int length;
	...
	{
		//LOGI_print("framer audio pts:%llu remains:%d length:%d \n", info.pts, shm_stream_remains(fShmSource), length);
		fFrameSize = length;
		...
		if(remains > 5)
			LOGI_print("framer audio pts:%llu length:%d fFrameSize:%d remains:%d", info.pts, length, fFrameSize, remains);
