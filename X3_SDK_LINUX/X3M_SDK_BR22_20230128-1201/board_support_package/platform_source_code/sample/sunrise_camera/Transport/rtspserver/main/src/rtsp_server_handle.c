...
int rtsp_server_init()
{
	...
	LOGI_print("handle->instance:%p", handle->instance);
...
int rtsp_server_start()
{
	...
	{
		LOGW_print("rtsp server alread start!");
		return 0;
	}
...
int rtsp_server_add_sms(void* param, unsigned int length)
{
	...
	rtspsvr_wrap_add_sms(handle->instance, sms_param->prefix,
		...
		sms_param->audio.bitspersample, sms_param->audio.channles,
	...
	printf("out rtsp_server_add_sms framerate:%d\n", sms_param->video.framerate);
