...
/* Host Send CSD command, cmd27 is received */
#define MMC_CSD_UPDATE			(1<<0)
/* Host Send CID command, cmd26 is received */
#define MMC_CID_UPDATE			(1<<1)
/* cmd23 is received */
#define MMC_SET_BLOCK_CNT		(1<<2)
/* cmd12 is received */
#define MMC_STOP_CMD			(1<<3)
/* Set device to idle,cmd0 is received */
#define MMC_IDLE_CMD			(1<<4)
/* Go inactive,cmd15 is received */
#define MMC_INACTIVE_CMD		(1<<5)
/* Set block length cmd16 is received */
#define MMC_SET_BLOCK_LEN		(1<<6)
/* cmd6 is received */
#define MMC_CMD6_ALWAYS			(1<<7)
/* Vendor cmd61 is received */
#define MMC_CMD_61			(1<<8)
/* Cmd40 is received */
#define MMC_CMD_40			(1<<9)
/* Read CMD17/18 is received */
#define MMC_BLK_READ			(1<<10)
/* Write CMD24/25 is received */
#define MMC_BLK_WRITE			(1<<11)
/* inform the successful completion of a write block transfer */
#define MMC_WRITE_BLOCK_CNT		(1<<12)
/* Inform successful completion of a read block transfer */
#define MMC_READ_BLOCK_CNT		(1<<13)
#define MMC_VOLTAGE_SWITCH		(1<<14)
/* MMC CMD8 or SD CMD11 is received, Switch 3.3V to 1.8V */
#define MMC_SPEED_CLASS_CTL		(1<<15)
/* Speed class control CMD20 is received */
#define MMC_CMD55			(1<<16)
/* CMD55 is received */
#define MMC_NUM_WELL_WRITE_BLOCK	(1<<17)
/* Start address of data to be erased CMD32 or CMD35 received */
#define MMC_START_ADDR_ERASE		(1<<18)
/* End address to be erased CMD33 or CMD38 received */
#define MMC_END_ADDR_ERASE		(1<<19)
/* Erase CMD38 is received */
#define MMC_ERASE_CMD			(1<<20)
/* Erase the entire card contents bit is set in received CMD42 */
#define MMC_FORCE_ERASE			(1<<21)
/* New password reception bit is set in the received CMD42 */
#define MMC_SET_PASSWD			(1<<22)
/* when clear the password contents bit is set in the received CMD42.*/
#define MMC_CLEAR_PASSWD		(1<<23)
/* when lock the card bit is set in the received CMD42*/
#define MMC_LOCK_CARD			(1<<24)
/* unlock the card bit is set in the received CMD42 */
#define MMC_UPLOCK_CARD			(1<<25)
/* set write protect command [CMD28] is received */
#define MMC_WRITE_PROTECT		(1<<26)
/* clear write protect command [CMD29] is received */
#define MMC_CLEAR_PROTECT		(1<<27)
/* send_write_prot command to get write protection
 * group status [CMD30] is received
*/
#define MMC_WRITE_PROT_STATUS		(1<<28)
/* general read/write command [CMD56] is received */
#define MMC_GENERAL_READ_WRITE		(1<<29)

/* This interrupt bit is set when CMD23 reception is not followed by
 the reception of CMD18 or CMD25. This interrupt informs the firmware
 to clear the set block count
*/
...
/* when the host signals the boot stop operation by making the command line
 * high.
*/
...
/* This interrupt bit is set when sleep/awake bit is cleared by the received
 * command [CMD5]
*/
...
/* This interrupt bit is set to stop the data transfer when the card is in
 * programming state
*/
...
/* This bit is set when CMD7 is received with card RCA to move the card to
 * transfer state
*/
#define MMC_CARD_SELECT		           (1<<9)

/* This bit is set when CMD7 is received with different RCA other than card
 * RCA to move the card to stand-by state*/
#define MMC_CARD_DESELECT		       (1<<10)

/*This interrupt bit is set when sanitize start bit is set in the received
 * CMD6
*/
...
/* his interrupt bit is set when tcase support interrupt bit is set in the
 * received CMD6
*/
#define MMC_TCASE_SUPPORT		       (1<<13)

/* This interrupt bit is set when packed command bit is set the received
 * CMD23
*/
...
/* This interrupt bit is set when Partition setting completed bit is set in the
 * received CMD6
*/
...
/* This interrupt bit is set when the card completes the packed data
 * transfer
*/
...
/* This interrupt bit is set when there is any change in the SD eMMC card state
 * register
*/
...
extern void hb_bifsd_initialize(void);
/*****************************************************************************/
/* INLINE Functions                                                          */
/*****************************************************************************/
...
static inline void bifsd_config_timing(void)
{
    ...
    reg_val = (NAC_NON_HS_200_400_VAL | (NAC_HS_200_400_VAL << 4)
