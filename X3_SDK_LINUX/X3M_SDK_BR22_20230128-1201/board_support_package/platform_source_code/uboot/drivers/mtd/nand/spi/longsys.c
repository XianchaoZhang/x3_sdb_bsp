...
static int f35uqa51m_ooblayout_ecc(struct mtd_info *mtd, int section,
...
static int f35uqa51m_ooblayout_free(struct mtd_info *mtd, int section,
				    struct mtd_oob_region *region)
{
	if (section > 3)
		return -ERANGE;
...
static const struct mtd_ooblayout_ops f35uqa51m_ooblayout = {
	.ecc = f35uqa51m_ooblayout_ecc,
	.free = f35uqa51m_ooblayout_free,
};
...
static const struct spinand_info longsys_spinand_table[] = {
	SPINAND_INFO("F35UQA512M", 0xCD,
		     ...
		     SPINAND_HAS_QE_BIT,
		     SPINAND_ECCINFO(&f35uqa51m_ooblayout,
				     NULL)),
