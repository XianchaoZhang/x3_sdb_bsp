			      ...
			      struct dwc3_generic_priv *priv)
{
	...
	priv->base_sys = map_physmem(plat->base_sys, SYSCTL_REGS_END, MAP_NOCACHE);
	dwc3->regs_sys = priv->base_sys;
...
static int dwc3_generic_ofdata_to_platdata(struct udevice *dev)
{
	...
	plat->base_sys = devfdt_get_addr_index(dev, 1);
...
static int dwc3_glue_probe(struct udevice *dev)
{
	...
	glue->regs_sys = dev_read_addr_index(dev, 1);
