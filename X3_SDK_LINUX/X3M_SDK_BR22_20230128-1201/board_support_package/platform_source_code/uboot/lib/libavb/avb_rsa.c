...
void reverse_key(uint8_t *key_buf, int key_size)
{
	uint8_t temp = 0;
	int i = 0;

	for(i = 0 ; i < key_size; i = i + 4) {
		temp = key_buf[i];
		key_buf[i] = key_buf[i + 3];
		key_buf[i + 3] = temp;

		temp = key_buf[i + 1];
		key_buf[i + 1] = key_buf[ i + 2 ];
		key_buf[i + 2] = temp;
	}
}
#endif
...
static bool x3_ras_pubkey_decryped(const uint8_t* key,
	...
	bool ret = false;
	uint8_t *key_buf = memalign(SPACC_ALIGN, 256);
	uint8_t *sig_buf = memalign(SPACC_ALIGN, 256);
	uint8_t *dest_buf = memalign(SPACC_ALIGN, 256);

	memset(key_buf, 0, 256);
	memset(sig_buf, 0, 256);
	memset(dest_buf, 0, 256);

	memcpy(key_buf, key + sizeof(AvbRSAPublicKeyHeader), RSA_PUB_KEY_SIZE);
	memcpy(sig_buf, sig, sig_num_bytes);

	/* rsa verify */
	dcache_disable();
	PKA_public_decrypt(RSA_PUB_KEY_SIZE, sig_buf, key_buf, dest_buf);
	dcache_enable();

	if (!memcmp((dest_buf + RSA_PUB_KEY_SIZE - HASH_SIZE), hash, HASH_SIZE))
		ret = true;
